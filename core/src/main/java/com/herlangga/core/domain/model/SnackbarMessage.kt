...
sealed interface SnackbarMessage {
    ...
    companion object {
        ...
        fun showErrorMessage(message: UiText): SnackbarMessage {
            return from(
                snackbarVisuals = CustomSnackbarVisuals(
                    uiText = message,
                    state = CustomSnackbarVisuals.SnackbarState.ERROR
                )
            )
        }

        fun showSuccessMessage(message: UiText): SnackbarMessage {
            return from(
                snackbarVisuals = CustomSnackbarVisuals(
                    uiText = message,
                    state = CustomSnackbarVisuals.SnackbarState.SUCCESS
                )
            )
        }
    }
